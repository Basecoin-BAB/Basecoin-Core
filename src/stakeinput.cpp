...
// The zBAB block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBAB is a hash of the serial
...
{
    //Create an output returning the zBAB that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBABOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBAB output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBAB", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBABOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBAB output", __func__);
...
{
    CzBABTracker* zbabTracker = pwallet->zbabTracker.get();
    CMintMeta meta;
    if (!zbabTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbabTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BAB stake is the outpoint
