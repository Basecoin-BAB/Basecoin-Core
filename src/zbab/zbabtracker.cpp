...
CzBABTracker::CzBABTracker(std::string strWalletFile)
{
...
CzBABTracker::~CzBABTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBABTracker::Init()
{
...
bool CzBABTracker::Archive(CMintMeta& meta)
{
...
bool CzBABTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBABTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBABTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBABTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBABTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBABTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBABTracker::GetSerialHashes()
{
...
CAmount CzBABTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBABTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBABTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBABTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBABTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBABTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBABTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBABTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBABTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBABTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBABTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBABWallet* zBABWallet)
{
    bool iszBABWalletInitialized = (NULL != zBABWallet);
    CMintMeta meta;
    ...
    if (! iszBABWalletInitialized)
        zBABWallet = new CzBABWallet(strWalletFile);
    meta.isSeedCorrect = zBABWallet->CheckSeed(dMint);
    if (! iszBABWalletInitialized)
        delete zBABWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBABTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBABTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBABTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBABTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBABTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBABTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBABWallet* zBABWallet = new CzBABWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBABWallet);
        }
        delete zBABWallet;
        LogPrint("zero", "%s: added %d dzbab from DB\n", __func__, listDeterministicDB.size());
    }
