...
CzBABWallet::CzBABWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbab seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBABSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBABSeed_deprecated()) {
                    LogPrintf("%s: Updated zBAB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbab seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbab wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBABWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBABCount(nCountLastUsed);
    else if (!walletdb.ReadZBABCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBABWallet::Lock()
{
    seedMaster = 0;
}

void CzBABWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBABWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBAB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBABWallet::LoadMintPoolFromDB()
{
...
void CzBABWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBABWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBABWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbabTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBABWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBAB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbabTracker which also adds to database
    pwalletMain->zbabTracker->Add(dMint, true);
...
void CzBABWallet::SeedToZBAB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBABWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBABWallet::GenerateDeterministicZBAB(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBABWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBABWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBABWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
