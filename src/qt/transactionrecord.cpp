...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZBABModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zbabTracker->HasMintTx(hash))) {
            //zBAB stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZBAB;
...
bool IsZBABType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZBAB:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZBAB || type == TransactionRecord::MNReward) {
