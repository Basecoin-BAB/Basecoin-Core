...
#include "ui_zbabcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbabTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbabTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBAB.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBAB seed used to mint this zBAB is not the same as currently hold in the wallet";
            else
