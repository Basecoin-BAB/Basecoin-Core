...
public:
    ...
    bool WriteZBABSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBABSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBABSeed_deprecated(uint256& seed);
    bool EraseZBABSeed();
    bool EraseZBABSeed_deprecated();

    bool WriteZBABCount(const uint32_t& nCount);
    bool ReadZBABCount(uint32_t& nCount);
