...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zbabspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BAB at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBAB send
enum ZerocoinSpendStatus {
    ZBAB_SPEND_OKAY = 0,                            // No error
    ZBAB_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBAB_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBAB_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBAB_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBAB_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBAB_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBAB_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBAB_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBAB_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBAB_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBAB_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBAB_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBAB_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBAB_SPENT_USED_ZBAB = 14,                      // Coin has already been spend
    ZBAB_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBAB_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZBABOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzbabAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzBABTracker> zbabTracker;
    ...
    void setZWallet(CzBABWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbabTracker = std::unique_ptr<CzBABTracker>(new CzBABTracker(strWalletFile));
    }

    CzBABWallet* getZWallet() { return zwalletMain; }
    ...
    /** zBAB reset */
    boost::signals2::signal<void()> NotifyzBABReset;
