...
- [Development guidelines](#development-guidelines)
    - [General Basecoin Core](#general-basecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Basecoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BABs (tBAB)" on the test network, if you
...
Basecoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/basecoin-project/basecoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_basecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_basecoin --log_level=test_suite
$ valgrind -v --leak-check=full src/basecoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_basecoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates BABs (if wallet is enabled).
...
However, in open source software such as Basecoin Core, where everyone uses
...
pay attention to for reviewers of Basecoin Core code.

General Basecoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Basecoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Basecoin Core so that they can be integrated
...
file descriptors it will cause problems with Basecoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Basecoin-Project/Basecoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `basecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `basecoin-cli` and the GUI debug console use this table to determine how to
